...
function drawEventSetup() {
  ...
  const medleyCounts = {
    Y: attendanceData.filter(a => a.present && a.special_event_entry === 'Y').length,
    Back: attendanceData.filter(a => a.present && a.special_event_entry === 'Back').length,
    Breast: attendanceData.filter(a => a.present && a.special_event_entry === 'Breast').length,
    Free: attendanceData.filter(a => a.present && a.special_event_entry === 'Free').length,
    N: attendanceData.filter(a => a.present && (a.special_event_entry === 'N' || !a.special_event_entry)).length,
  };
  ...
  el.innerHTML = `
    ...
    <div class="card" style="padding:16px;margin-bottom:16px;background:#fff8e1;border-left:8px solid #f59e0b">
      ...
      ${isMedley ? `<div style="margin-top:10px;padding-top:10px;border-top:1px solid #f6c453;font-size:16px;color:#5d4037"><strong>Medley Assigned:</strong> Back: ${medleyCounts.Back} | Breast: ${medleyCounts.Breast} | Free: ${medleyCounts.Free} | Yes (auto): ${medleyCounts.Y} | No: ${medleyCounts.N}</div>` : ''}
    </div>
...
function renderAttRow(a, idx) {
  ...
  return `
    <tr>
      ...
      <td class="time-cell">${formatWhole(a.time_25m)}</td>
      <td class="time-cell">${formatWhole(a.time_50m)}</td>
      <td class="time-cell">${formatWhole(a.time_75m)}</td>
      <td class="time-cell">${formatWhole(a.time_backstroke)}</td>
      <td class="time-cell">${formatWhole(a.time_breaststroke)}</td>
      <td class="time-cell">${formatWhole(a.time_butterfly)}</td>
      ${entryCell}
    </tr>
  `;
}
...
// BF2.6-04: Tab should move to next input/select field in spreadsheet-style flow
document.addEventListener('keydown', function(e) {
  if (e.key !== 'Tab') return;
  const active = document.activeElement;
  if (!active) return;
  if (!active.matches('input, select, textarea, button')) return;
  ...
  e.preventDefault();
  const next = fields[(idx + (e.shiftKey ? -1 : 1) + fields.length) % fields.length];
  if (next) next.focus();
});
