...
**Substantive anchors:** `219bdd9` (engine + API + UI), `b3f9a82` (CSV-route refactor + test suite + specs + SSOT), `b598648` (first clean-HEAD evidence), and `a94c0fc` (Balerion-QA hardening: unknown-race_type fix + UT9, M2 HEAD-staleness gate, mobile-screenshot fix, TC-069 wording, 75m/breaststroke/butterfly seeding). The latest clean-HEAD evidence package was generated at HEAD `a94c0fc`; the following docs-only evidence commit stores it and becomes the new HEAD (SSOT-anchor pattern used since v2.8.6 / R-M3-05). HEAD is dynamic — resolve with `git rev-parse --short HEAD`.
**Date:** 2026-06-03
