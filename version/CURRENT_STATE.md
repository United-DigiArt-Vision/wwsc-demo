...
**WorkingTreeStatus:** M3 pointscore implemented under Bryan's 2026-06-02 working assumptions (event-separated; monthly/season by simple addition; Excel pointscore sheets as working source; adjustable later — NOT confirmed Constitution). Balerion independently reproduced on 2026-06-04: unit 12 PASS / 0 FAIL (incl. UT9 unknown-race_type); isolation PASS (accepted flow byte-identical with pointscore on/off; deterministic 46/46/6); 120-case browser suite 114 PASS / 6 NA / 0 FAIL / 0 BLOCKED on fresh M2 logs at `711c66d`; M2 regression 55/0 and 98/2/0/0; R-M3-05 history-graphs 19/1/0. Balerion caught that Claude's claimed clean UIT-M3-113 mobile screenshot still showed an Event Details overlay, fixed the harness cleanup in `711c66d`, reran evidence, and visually confirmed `UIT-M3-113-resp-mobile.png` is now clean. Determinism boundary documented (per-swimmer attribution varies via the accepted v2.8.9 randomized heat builder; all assertions are structural invariants). Constitution-specific accumulation (R-M3-03) deferred and isolated.
**Milestone status:** M2 v2.9.0 RELEASED/PAID and protected. M3 pointscore slice IMPLEMENTED + Balerion V0015 QA PASS on the M3 branch; release/deploy/client communication still requires explicit Dino authorization. No push, no deploy, no tag, no Bryan/client contact, no live-data mutation.
